...
public class ExportSignedPackage implements Tool {
  ...
  protected void promptSigningKey() {
    ...
    layout =
        (ScrollView)
            LayoutInflater.from(context).inflate(R.layout.export_signed_package, null, false);
    ...
    builder.setPositiveButton(
        R.string.export_signed_package_export, (dialog, which) -> releaseBuild());
                ...
                .setOnClickListener(
                    v -> {
                      AlertDialog.Builder infoBuilder =
                          new AlertDialog.Builder(context.getEditor());
    ...
    componentTargetAdapter =
        new ArrayAdapter<String>(context, R.layout.spinner_item, componentTargetList);
    ...
    keystoreFileSelect.setOnClickListener(
        view -> {
    ...
    keystoreFile.addTextChangedListener(
        new TextWatcher() {
          @Override
          public void afterTextChanged(Editable arg0) {
            ...
            aliasAdapter.clear();
            aliasAdapter.add(
                context.getResources().getString(R.string.export_signed_package_no_aliases));
            aliasPassword.setEnabled(false);
                              ...
                              () -> {
                                if (keystoreFileValidation.isNewestTicket(ticket)) {
                                  ...
                                  if (result.resultCode() == 0) {
                                    ...
                                    messageView(
                                        exportMessage,
                                        ValidationResult.MessageSeverity.INFO,
                                        R.string
                                            .export_signed_package_info_enter_keystore_password);
                                  } else {
                                    keystorePassword.setEnabled(false);
    ...
    keystorePassword.addTextChangedListener(
        new TextWatcher() {
            ...
            if (keystorePassword.getText().length() > 0) {
                                    ...
                                    if (result.resultCode() == 0) {
                                      ...
                                      if (alias.isEnabled()) {
                                        messageView(
                                            exportMessage,
                                            ValidationResult.MessageSeverity.INFO,
                                            R.string.export_signed_package_info_enter_key_password);
                                        ...
                                        messageView(
                                            exportMessage,
                                            ValidationResult.MessageSeverity.INFO,
                                            R.string.export_signed_package_info_create_key);
                                      }
              ...
              if (keystorePasswordValidation.isNewestTicket(ticket)) {
                messageView(
                    exportMessage,
                    ValidationResult.MessageSeverity.INFO,
                    R.string.export_signed_package_info_enter_keystore_password);
              }
            }
          }
    ...
    alias.setOnItemSelectedListener(
    ...
    aliasCertificateInfo.setOnClickListener(
        v -> {
          loadCertificate(
              keystoreFile.getText().toString(),
              getKeystoreFileUri(),
          ...
          MaterialAlertDialogBuilder infoBuilder =
              new MaterialAlertDialogBuilder(context.getEditor());
          ...
          infoLayout =
              (TableLayout)
                  LayoutInflater.from(context).inflate(R.layout.certificate_info, null, false);

          ((TextView) infoLayout.findViewById(R.id.alias_name))
              .setText((String) alias.getSelectedItem());
          ((TextView) infoLayout.findViewById(R.id.certificate_expiration))
              .setText(
                  new SimpleDateFormat(
                          context
                              .getResources()
                              .getString(R.string.export_signed_package_date_format),
                          Locale.US)
          ...
          if (principal instanceof X500Principal) {
            ...
            X509Principal x509 =
                new X509Principal(
                    x500Principal.getName()); // Because X509Principal parses the String for us

            ((TextView) infoLayout.findViewById(R.id.alias_certificate_name))
                .setText((String) x509.getValues(X509Name.CN).get(0));
            ((TextView) infoLayout.findViewById(R.id.alias_certificate_organizational_unit))
                .setText((String) x509.getValues(X509Name.OU).get(0));
            ((TextView) infoLayout.findViewById(R.id.alias_certificate_organization))
                .setText((String) x509.getValues(X509Name.O).get(0));
            ((TextView) infoLayout.findViewById(R.id.alias_certificate_city))
                .setText((String) x509.getValues(X509Name.L).get(0));
            ((TextView) infoLayout.findViewById(R.id.alias_certificate_state))
                .setText((String) x509.getValues(X509Name.ST).get(0));
            ((TextView) infoLayout.findViewById(R.id.alias_certificate_country))
                .setText((String) x509.getValues(X509Name.C).get(0));
          } else {
            // This seems to work with keystores generated by APDE... does that make that our
            // generator is broken?

            // Yuck, reflection... but Android hides the BouncyCastle APIs that our certificate
            // forces us to use
            try {
              Class<?> x509Principal =
                  Class.forName("com.android.org.bouncycastle.jce.X509Principal");
              Class<?> x509Name = Class.forName("com.android.org.bouncycastle.asn1.x509.X509Name");
              Class<?> ans1Ident =
                  Class.forName("com.android.org.bouncycastle.asn1.ASN1ObjectIdentifier");
                ...
                ((TextView) infoLayout.findViewById(R.id.alias_certificate_name))
                    .setText(
                        (String)
                            ((Vector)
                                    getValues.invoke(
                                        x509Principal.cast(principal),
                                        x509Name.getField("CN").get(null)))
                                .get(0));
                ((TextView) infoLayout.findViewById(R.id.alias_certificate_organizational_unit))
                    .setText(
                        (String)
              ...
              System.err.println(
                  context
                      .getResources()
                      .getString(R.string.export_signed_package_certificate_read_failed));
              e.printStackTrace();
            }
          }
    ...
    aliasPassword.addTextChangedListener(
        new TextWatcher() {
          @Override
                                      ...
                                      messageView(
                                          ...
                                          String.format(
                                              ...
                                              context
                                                  ...
                                                  .getString(
                                                      R.string
                                                          .export_signed_package_info_export_ready),
                                              ...
                                              new SimpleDateFormat(
                                                      context
                                                          ...
                                                          .getString(
                                                              R.string
                                                                  .export_signed_package_date_format),
                                                      Locale.US)
                                                  .format(certificate.getNotAfter())));
                                    } else {
                                      exportButton.setEnabled(false);
    ...
    messageView(
        exportMessage,
        ValidationResult.MessageSeverity.INFO,
        R.string.export_signed_package_info_enter_keystore_file);

    if (keystoreAdapter.getCount() > 0) {
      // Automatically select the first keystore in the list - if the user wants to change it, they
      // can delete the text...
  ...
  protected void promptCreateKeystore() {
    ...
    layout =
        (ScrollView) LayoutInflater.from(context).inflate(R.layout.create_keystore, null, false);
    ...
    builder.setPositiveButton(
        ...
        (dialog, which) -> {
          writeEmptyKeystore(
              createKeystoreFile.getText().toString(),
              getCreateKeystoreFileUri(),
              createKeystorePassword.getText().toString().toCharArray());
    ...
    createKeystoreFileSelect.setOnClickListener(
        view -> {
    ...
    TextWatcher createKeystoreValidator =
        new TextWatcher() {
          @Override
          public void afterTextChanged(Editable arg0) {
            // This is ugly... so what?
            if (createKeystoreFile.getText().length() > 0) {
              if (createKeystorePassword.getText().length() > 0
                  && createKeystorePasswordConfirm.getText().length() > 0) {
                if (createKeystorePasswordConfirm
                    .getText()
                    .toString()
                    .equals(createKeystorePassword.getText().toString())) {
                  ...
                  if (createKeystorePassword.getText().length() < 8) {
                    ...
                    messageView(
                        keystoreMessage,
                        ValidationResult.MessageSeverity.WARNING,
                        R.string.export_signed_package_warning_short_password);
                  } else {
                    messageView(
                        keystoreMessage,
                        ValidationResult.MessageSeverity.INFO,
                        String.format(
                            ...
                            context
                                .getResources()
                                .getString(
                                    R.string.export_signed_package_info_create_keystore_ready),
                            context
                                .getResources()
                                .getString(
                                    isBksKeystore(keystoreUri)
                                        ? R.string.export_signed_package_keystore_type_bks
                                        : R.string.export_signed_package_keystore_type_jks)));
                  }
                  ...
                  messageView(
                      keystoreMessage,
                      ValidationResult.MessageSeverity.ERROR,
                      R.string.export_signed_package_error_passwords_disagree);
                }
              } else {
                messageView(
                    keystoreMessage,
  ...
  protected void promptCreateKey() {
    MaterialAlertDialogBuilder builder = new MaterialAlertDialogBuilder(context.getEditor());
    ...
    final EditText certificateOrganizationalUnit =
        layout.findViewById(R.id.create_alias_certificate_organizational_unit);
    final EditText certificateOrganization =
        layout.findViewById(R.id.create_alias_certificate_organization);
    ...
    builder.setPositiveButton(
        ...
        (dialog, which) -> {
          ...
          writeKey(
              ...
              getKeystoreFileUri(),
              keystorePassword.getText().toString().toCharArray(),
              createAliasAlias.getText().toString(),
              createAliasPassword.getText().toString().toCharArray(),
              Integer.parseInt(createAliasValidity.getText().toString()),
              certificateName.getText().toString(),
              certificateOrganizationalUnit.getText().toString(),
              certificateOrganization.getText().toString(),
              certificateCity.getText().toString(),
              certificateState.getText().toString(),
              certificateCountry.getText().toString());
    ...
    TextWatcher createKeyValidator =
        new TextWatcher() {
          @Override
          public void afterTextChanged(Editable arg0) {
            if (createAliasAlias.getText().length() > 0) {
              if (createAliasPassword.getText().length() > 0
                  && createAliasPasswordConfirm.getText().length() > 0) {
                if (createAliasValidity.getText().length() > 0) {
                  if (createAliasPassword
                      .getText()
                      .toString()
                      .equals(createAliasPasswordConfirm.getText().toString())) {
                    ...
                    if (containsAlias) {
                      messageView(
                          keyMessage,
                          ValidationResult.MessageSeverity.WARNING,
                          R.string.export_signed_package_warning_key_exists);
                    } else if (Integer.parseInt(createAliasValidity.getText().toString()) < 25) {
                      messageView(
                          keyMessage,
                          ValidationResult.MessageSeverity.WARNING,
                          R.string.export_signed_package_warning_certificate_validity_short);
                      ...
                      messageView(
                          keyMessage,
                          ValidationResult.MessageSeverity.WARNING,
                          R.string.export_signed_package_warning_certificate_empty);
                    } else {
                      messageView(
                          keyMessage,
                          ValidationResult.MessageSeverity.INFO,
                          R.string.export_signed_package_info_create_key_ready);
                    }
                  ...
                  messageView(
                      keyMessage,
                      ValidationResult.MessageSeverity.INFO,
                      R.string.export_signed_package_info_enter_certificate_validity);
                }
              } else {
                messageView(
                    keyMessage,
                    ValidationResult.MessageSeverity.INFO,
                    R.string.export_signed_package_info_enter_alias_password);
              }
            } else {
              messageView(
                  keyMessage,
                  ValidationResult.MessageSeverity.INFO,
                  R.string.export_signed_package_info_enter_alias);
            }
  ...
  protected ValidationResult validateKeystoreFile() {
    ...
    ParcelFileDescriptor fd =
        FileSelection.openUri(context, keystoreUri, FileSelection.Mode.READ, true);
      ...
      if (fd != null) {
        ...
        if (keystoreFile.getText().length() > 0) {
          return new ValidationResult(
              1,
              ValidationResult.MessageSeverity.ERROR,
              R.string.export_signed_package_error_keystore_file_nonexistant);
        } else {
          return new ValidationResult(
              2,
              ValidationResult.MessageSeverity.INFO,
      ...
      try {
        ...
        result =
            new ValidationResult(
                2,
                ValidationResult.MessageSeverity.ERROR,
                R.string.export_signed_package_error_keystore_not_recoverable);
        ...
        try {
          ...
          // Is this a bug in BouncyCastle? Is it because files starting with dots are hidden? Or is
          // there something wrong with this particular ".nomedia" file?
          e.printStackTrace();
        } finally {
          try {
            if (in != null) {
  ...
  protected ValidationResult loadCertificate(
      String keystoreUriPath, Uri keystoreUri, char[] keystorePassword, String alias) {
    ...
    if (keystoreResult.resultCode() != 0) {
      ...
      try {
        if (keystore.containsAlias(alias)) {
          ...
          // We just need to make sure that we can load these - we don't actually have to use
          // them...
  ...
  protected ValidationResult loadKey(
      String keystoreUriPath,
      Uri keystoreUri,
      ...
      try {
        ...
        result =
            new ValidationResult(
                3,
                ValidationResult.MessageSeverity.ERROR,
                R.string.export_signed_package_error_key_not_recoverable);
        ...
        result =
            new ValidationResult(
                ...
                String.format(
                    ...
                    context
                        .getResources()
                        .getString(R.string.export_signed_package_error_certificate_expired),
                    new SimpleDateFormat(
                            context
                                .getResources()
                                .getString(R.string.export_signed_package_date_format),
        ...
        result =
            new ValidationResult(
                ...
                String.format(
                    ...
                    context
                        .getResources()
                        .getString(R.string.export_signed_package_error_certificate_not_yet_valid),
                    new SimpleDateFormat(
                            context
                                .getResources()
                                .getString(R.string.export_signed_package_date_format),
  ...
  protected void writeKeystore(String uriPath, Uri uri, char[] password) {
    ParcelFileDescriptor fd =
        FileSelection.openUri(context, uri, FileSelection.Mode.READ_WRITE, true);
    if (fd == null) {
  ...
  protected void writeKey(
    ...
    try {
      ...
      v3CertGen.setNotAfter(
          new Date(
              System.currentTimeMillis()
                  + (1000L * 60 * 60 * 24 * 365
                      * validity))); // TODO Doesn't take leap days / years into account...
      ...
      loadAliases(
          (ArrayList<String>) loadKeystore(keystoreUriPath, keystoreUri, keystorePassword).extra());
  ...
  private String formatDN(String value) {
    ...
    final char[] escapeChars = {
      '\\', '"', '#', '+', ',', ';', '=', '<', '>'
    }; // Note: "\" character must be escaped first...
    ...
    return value.length() > 0
        ? value
        : context.getString(R.string.export_signed_package_certificate_field_empty_default);
  }
  ...
  public void putRecentKeystore(String path) {
    ...
    PreferenceManager.getDefaultSharedPreferences(context)
        .edit()
        .putString("recentKeystores", data)
        .apply();
  }

  public ArrayList<String> getRecentKeystores() {
    String data =
        PreferenceManager.getDefaultSharedPreferences(context).getString("recentKeystores", "");
  ...
  public static final class ValidationResult {
    ...
    public ValidationResult(
        int resultCode, MessageSeverity severity, String message, Object extra) {
      this.resultCode = resultCode;
      this.severity = severity;
      this.message = message;
      this.extra = extra;
  ...
  private void messageView(
      TextView messageView, ValidationResult.MessageSeverity severity, int messageId) {
  ...
  private void messageView(
      TextView messageView, ValidationResult.MessageSeverity severity, String message) {
    if (message.length() == 0) {
  ...
  protected void releaseBuild() {
    ...
    final File binFolder =
        new File(
            (context.isExample() || context.isTemp())
                ? context.getSketchbookFolder()
                : context.getSketchLocation(),
            "bin");
    ...
    new Thread(
            () -> {
              ...
              builder.build(
                  "release",
                  ComponentTarget.deserialize(componentTarget.getSelectedItemPosition()));
              exporting = false;
            })
        .start();
  }
