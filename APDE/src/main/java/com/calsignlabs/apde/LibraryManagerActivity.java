...
public class LibraryManagerActivity extends AppCompatActivity {
  // All potential archive / compressed file extensions (hopefully) TODO Yes, I realize that the
  // list currently contains one item.
    ...
    switch (requestCode) {
      case REQUEST_CHOOSER:
        if (resultCode == RESULT_OK) {
          ...
          if (libraryName == null) {
            alert(
                getResources().getString(R.string.library_manager_install_invalid_file_error_title),
                "Could not detect library name");
          } else if (new Library(libraryName)
              .getLibraryFolder(((APDE) getApplicationContext()).getLibrariesFolder())
              .exists()) {
            alert(
                getResources().getString(R.string.library_manager_install_invalid_file_error_title),
                getResources()
                    .getString(
                        R.string
                            .library_manager_install_invalid_file_error_message_already_installed));
          } else {
  ...
  public boolean onOptionsItemSelected(MenuItem item) {
    if (item.getItemId() == android.R.id.home) {
      finish();
      return true;
    } else if (item.getItemId() == R.id.action_install_zip_library) {
      launchInstallZipLibrary();
      return true;
    } else if (item.getItemId() == R.id.action_get_libraries) {
      launchGetLibraries();
      return true;
    } else if (item.getItemId() == R.id.action_settings) {
      launchSettings();
      return true;
    } else if (item.getItemId() == R.id.action_dx_dexer_tool) {
      launchDexerTool();
      return true;
    } else {
  ...
  private void launchInstallZipLibrary() {
    Intent intent =
        FileSelection.createFileSelectorIntent(
            false,
            new String[] {
              "application/zip",
  ...
  private void launchGetLibraries() {
    // Open a browser to Processing's libraries page so that the user can download libraries
    // manually
    // This is until we figure out what to do about downloading and installing libraries directly

    Intent browserIntent =
        new Intent(Intent.ACTION_VIEW, Uri.parse("http://processing.org/reference/libraries/"));
  ...
  private void launchDexerTool() {
    ...
    layout = (ScrollView) LayoutInflater.from(this).inflate(R.layout.dx_dexer_tool, null, false);
    ...
    layout
        .findViewById(R.id.dx_dexer_input_file_select)
        .setOnClickListener(
            v -> {
              Intent intent =
                  FileSelection.createFileSelectorIntent(
                      false, new String[] {"application/java-archive"});
              startActivityForResult(intent, DX_DEXER_SELECT_INPUT_FILE);
            });

    layout
        .findViewById(R.id.dx_dexer_output_file_select)
        .setOnClickListener(
            v -> {
              ...
              if (inputFilename.endsWith(".jar")) {
                outputFilename =
                    inputFilename.substring(0, inputFilename.length() - 4) + "-dex.jar";
              } else {
    ...
    dxDexerInputFile.addTextChangedListener(
        new TextWatcher() {
          @Override
    ...
    dxDexerOutputFile.addTextChangedListener(
        new TextWatcher() {
          @Override
          public void afterTextChanged(Editable arg0) {
            validateDxDexerFiles();
    ...
    dxDexerDexButton.setOnClickListener(
  ...
  private void dxDexerRun() {
    ...
    final Uri outputUri =
        FileSelection.uriStringOrPathToUri(dxDexerOutputFile.getText().toString());
    ...
    dialog.setTitle(
        getResources().getString(R.string.dx_dexer_dialog_title)
            + " "
            + FileSelection.uriToFilename(this, inputUri));
    dialog.setCanceledOnTouchOutside(false);
    dialog.setCancelable(false);

    final Thread dexThread =
        new Thread(
            () -> {
              working = true;
              ...
              try {
                ...
                inputFd =
                    FileSelection.openUri(
                        LibraryManagerActivity.this, inputUri, FileSelection.Mode.READ);
                if (!isOutputHardPath) {
                  outputFd =
                      FileSelection.openUri(
                          LibraryManagerActivity.this, outputUri, FileSelection.Mode.READ_WRITE);
                }
    ...
    dialog.setProgressText(
        getResources().getString(R.string.library_manager_install_message_dexing) + "...");
  }
  ...
  public void refreshLibraryList() {
    ...
    final LibraryManagerAdapter adapter =
        new LibraryManagerAdapter(
            this,
            R.layout.library_manager_list_item,
            ((APDE) getApplicationContext()).getLibraries());
      ...
      if (view != null && view instanceof RelativeLayout) {
        ...
        author.setText(
            Html.fromHtml(
                toHtmlLinks(
                    String.format(
                        getApplication().getString(R.string.library_manager_library_author_by),
                        lib.getAuthorList(
                            ((APDE) getApplicationContext()).getLibrariesFolder())))));
        desc.setText(
            Html.fromHtml(
                toHtmlLinks(
                    lib.getSentence(((APDE) getApplicationContext()).getLibrariesFolder()))));
        ...
        ImageButton button =
            (ImageButton) container.findViewById(R.id.library_manager_list_item_actions);
        ...
        button.setOnClickListener(
            new View.OnClickListener() {
              ...
              public void onClick(View button) {
                ...
                // This is a custom implementation, designed to support API level 10+ (Android's
                // PopupMenu is 11+)
                ...
                popup.setOnMenuItemClickListener(
                    new PopupMenu.OnMenuItemClickListener() {
                      ...
                      public boolean onMenuItemClick(MenuItem item) {
                        if (item.getItemId() == R.id.menu_contrib_uninstall) {
  ...
  // This is straight from the Processing source - the class is default access, so we can't get to
  // it from here
  /**
   ...
   * <p>Becomes...
   *
   * <p>This has a <a href="http://example.com/">link</a> in <a href="http://example.org/">it</a>.
   */
  ...
  private void launchUninstallDialog(final Library lib) {
    ...
    AlertDialog.Builder builder = new AlertDialog.Builder(this);
    builder.setTitle(
        getResources().getString(R.string.library_manager_contrib_uninstall_warning_title)
            + " "
            + lib.getName());
    builder.setMessage(
        lib.getName()
            + " "
            + getResources().getString(R.string.library_manager_contrib_uninstall_warning_message));

    builder.setPositiveButton(
        R.string.library_manager_contrib_uninstall_warning_title,
        (dialog, which) -> uninstallLibrary(lib));
            ...
            new Runnable() {
              ...
              public void run() {
                switch ((Library.Status) msg.obj) {
                  case COPYING:
                    dialog.setProgressText(
                        res.getString(R.string.library_manager_install_message_copying) + "...");
                    break;
                  case EXTRACTING:
                    dialog.setProgressText(
                        res.getString(R.string.library_manager_install_message_extracting) + "...");
                    break;
                  case DEXING:
                    dialog.setProgressText(
                        res.getString(R.string.library_manager_install_message_dexing) + "...");
                    break;
                  case INSTALLED:
  ...
  private void addZipLibrary(final String libraryName, final Uri uri) {
    ...
    final Thread installThread =
        new Thread(
            () -> {
              working = true;
              boolean success =
                  ContributionManager.installZipLibrary(
                      library, uri, libraryUpdateHandler, context);
              working = false;
              ...
              if (!success) {
                runOnUiThread(
                    () ->
                        alert(
                            R.string.library_install_failure_title,
                            R.string.library_install_failure_message));
              }

              findViewById(R.id.library_manager_list)
                  .post(
                      () -> {
    ...
    // Set up the progress dialog TODO It would be cool if this was a real progress bar, displaying
    // percentage complete -
    // but we can't get enough out of DEX to pull this off (and the dexer takes almost all of the
    // time...)
    dialog.setProgressStyle(ProgressDialog.STYLE_HORIZONTAL);
    dialog.setIndeterminate(true);
    dialog.setTitle(
        getResources().getString(R.string.library_manager_install_dialog_title)
            + " "
            + libraryName);
    dialog.setCanceledOnTouchOutside(false);
    dialog.setCancelable(false);
  ...
  private void uninstallLibrary(final Library library) {
    ...
    dialog.setIndeterminate(true);
    dialog.setTitle(
        getResources().getString(R.string.library_manager_library_uninstall_dialog_title)
            + " "
            + library.getName());
    dialog.setCanceledOnTouchOutside(false);
    dialog.setCancelable(false);
    ...
    dialog.setButton(
        ...
        getResources().getString(R.string.task_run_in_background),
        new DialogInterface.OnClickListener() {
          @Override
          public void onClick(DialogInterface dialog, int which) {
            dialog.dismiss();
