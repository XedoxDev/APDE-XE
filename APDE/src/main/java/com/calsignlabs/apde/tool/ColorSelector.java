...
public class ColorSelector implements Tool {
  ...
  public void init(APDE context) {
    ...
    // "0.5f" scaling factor is neccessary because the graphics were originally created for a
  ...
  public void run() {
    if (dialog == null) {
      ...
      layout =
          (LinearLayout) LayoutInflater.from(context).inflate(R.layout.color_selector, null, false);
          ...
          new Runnable() {
            public void run() {
              LinearLayout.LayoutParams params =
                  (LinearLayout.LayoutParams) hueStrip.getLayoutParams();
              ...
              setHue(
                  (1.0f
                          - Math.min(
                              Math.max(
                                  0.0f,
                                  (event.getY() - 12.0f * DIP)
                                      / (hueStrip.getHeight() - 24.0f * DIP)),
                              1.0f))
                      * 360);
              return true;
            }
      ...
      colorSquare.setOnTouchListener(
          new View.OnTouchListener() {
            @Override
            public boolean onTouch(View view, MotionEvent event) {
              setSaturation(Math.min(Math.max(event.getX() / colorSquare.getWidth(), 0.0f), 1.0f));
              setBrightness(
                  1.0f - Math.min(Math.max(event.getY() / colorSquare.getHeight(), 0.0f), 1.0f));
              ...
              ((android.content.ClipboardManager)
                  ...
                  .setPrimaryClip(
                      android.content.ClipData.newPlainText(
                          context.getResources().getString(R.string.tool_color_selector_hex_color),
                          hex.getText()));

              Toast.makeText(
                      context.getEditor(),
                      R.string.tool_color_selector_copy_hex_to_clipboard_success,
                      Toast.LENGTH_SHORT)
                  .show();
            }
          });
  ...
  public void setRGB(float red, float green, float blue) {
    ...
    colorSquare.setPoint(
        hsb[1] * colorSquare.getWidth(), (1.0f - hsb[2]) * colorSquare.getHeight());
    ...
    if (selectionBackground == null) {
      selectionBackground =
          ((GradientDrawable)
              context.getResources().getDrawable(R.drawable.color_selector_selection_background));
    }
  ...
  public void setHSB(float hue, float saturation, float brightness) {
    ...
    colorSquare.setPoint(
        saturation * colorSquare.getWidth(), (1.0f - brightness) * colorSquare.getHeight());
  ...
  public static class HueStrip extends View {
    ...
    public void onDraw(Canvas canvas) {
      ...
      if (FLAG_REFRESH_HUE) {
        ...
        float drawHue =
            Math.min(
                Math.max((1.0f - hue) * getMeasuredHeight(), 12.0f * DIP),
                getMeasuredHeight() - 12.0f * DIP);

        canvas.drawRect(
            2.0f * DIP,
            drawHue - 10.0f * DIP,
            getMeasuredWidth() - 2.0f * DIP,
            drawHue + 10.0f * DIP,
            boxPaint);
      }
    }
  ...
  public static class ValueEditText extends EditText {
              ...
              if (!FLAG_INTERNAL_MODIFICATION) {
                callback.onValueChanged(
                    text.toString().length() == 0
                        ? 0.0f
                        : Integer.parseInt(text.toString()) / scale);
              }
            }
    ...
    public void setValue(float value) {
      ...
      if (!FLAG_MODIFIED // Make sure we set the text initially
          || !((text.equals("")
                  && hasFocus()) // Don't change the text to "0" if it's empty (if it has focus)
              || (text.length() > 0
                  && Integer.parseInt(text)
                      == (int)
                          (value * scale))) // Don't change the text if it will make no difference
